...
const MATRIX_BASE_URL: string = 'https://api.openrouteservice.org/v2/matrix/driving-car';
...
export async function getRouteMatrices(coordinates: Coordinate[]): Promise<{
    ...
    try {
        ...
        throw new Error(`Failed to fetch route matrices: ${error?.response?.data || error}`);
    }
}
