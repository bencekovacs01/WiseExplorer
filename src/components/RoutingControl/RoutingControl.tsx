...
const RoutingControl: React.FC<RoutingControlProps> = ({ positions }) => {
  ...
  useEffect(() => {
    ...
    const isPartOfCluster: boolean[] = new Array(positions.length).fill(false);
    ...
    const waypoints = positions
      ...
      .map((position) => L.latLng(position.coords.lat, position.coords.lng));
    ...
    positions.forEach((position, index) => {
      ...
      const marker = L.marker([position.coords.lat, position.coords.lng], {
        icon: markerIcon,
      }).addTo(map);
    ...
    routingControl.on('routesfound', (e: any) => {
      ...
      const instructionWaypoints = routes[0]?.instructions?.map?.(
        (instruction: IInstruction) => {
          const { lat, lng } = routes[0]?.coordinates?.[instruction?.index];
          return L.latLng(lat, lng);
        },
      );
