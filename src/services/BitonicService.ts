...
export class BitonicService {
  ...
  private async findBitonicRouteInternal(
    ...
    const { distanceMatrix, durationMatrix } =
      await this.getRouteMatrices(clusteredPois);
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (i < n - 1) {
        const origI =
          sortedToOriginalIndex.get(this.getPoiKey(sortedPois[i])) || i;
        const origJ =
          sortedToOriginalIndex.get(this.getPoiKey(sortedPois[i + 1])) || i + 1;
        ...
        if (
          ...
          throw new Error(`Invalid index: origI=${origI}, origJ=${origJ}`);
        }
    ...
    for (let l = 3; l <= n; l++) {
      for (let i = 0; i <= n - l; i++) {
        ...
        for (let k = i + 1; k < j; k++) {
          const origK =
            sortedToOriginalIndex.get(this.getPoiKey(sortedPois[k])) || k;
          const origJ =
            sortedToOriginalIndex.get(this.getPoiKey(sortedPois[j])) || j;
    ...
    for (let i = 0; i < route.length - 1; i++) {
      ...
      if (origI !== undefined && origJ !== undefined) {
        ...
        console.warn(`Coordinates not found in matrix: ${key1} or ${key2}`);
