...
class BranchAndBoundService {
        ...
        while (queue.length > 0) {
            ...
            for (let i = 0; i < n; i++) {
                if (!currentNode.visited.has(i)) {
                    const lastVisited =
                        currentNode.path[currentNode.path.length - 1];
                    const nextCost =
                        currentNode.cost + durationMatrix[lastVisited][i];
    ...
    private calculateLowerBound(
        ...
        for (let j = 0; j < totalPoints; j++) {
            if (
                !visited.includes(j) &&
                durationMatrix[current][j] < minOutgoing
            ) {
        ...
        if (visited.length > 1) {
            ...
            for (let i = 0; i < totalPoints; i++) {
                if (
                    !visited.includes(i) &&
                    durationMatrix[i][0] < minIncoming
                ) {
